...
class InstallationVerifier:
    ...
    def run_command(self, cmd: list, timeout: int = 30, capture_output: bool = True) -> Optional[subprocess.CompletedProcess]:
    ...
    def test_cli_entry_points(self) -> bool:
        ...
        result = self.run_command([sys.executable, "-m", "mcp_clipboard_server", "--help"])
        if result and result.returncode == 0:
            ...
            try:
                ...
                if stdout:
                    ...
                    if response.get("jsonrpc") == "2.0" and "result" in response:
                        self.log_test("MCP Protocol Init", True, "Initialize successful")
                        return True
                    else:
                        self.log_test("MCP Protocol Init", False, f"Invalid response: {response}")
                        return False
                else:
                    self.log_test("MCP Protocol Init", False, f"No stdout. stderr: {stderr}")
                    return False
                    
            except subprocess.TimeoutExpired:
                process.kill()
                self.log_test("MCP Protocol Init", False, "Timeout waiting for response")
                return False
            ...
            try:
                ...
                if stdout:
                    ...
                    if success_count >= 2:  # At least init and tools/list should work
                        self.log_test("Clipboard Tools", True, f"{success_count}/{len(requests)} requests succeeded")
                        return True
                    else:
                        self.log_test("Clipboard Tools", False, f"Only {success_count}/{len(requests)} requests succeeded")
                        return False
                else:
                    self.log_test("Clipboard Tools", False, f"No response. stderr: {stderr}")
                    return False
    ...
    def test_platform_detection(self) -> bool:
        ...
        try:
            from mcp_clipboard_server.clipboard import _get_platform_info, _get_platform_guidance
            ...
            if platform_info and guidance:
                ...
                self.log_test("Platform Detection", False, "Platform detection returned empty")
                return False
        ...
        try:
            ...
            try:
                ...
                if result == unicode_text:
                    ...
                    self.log_test("Unicode Support", True, "Graceful fallback to empty string")
                    return True
                else:
                    self.log_test("Unicode Support", False, f"Unexpected result: {repr(result)}")
                    return False
    ...
    def test_error_handling(self) -> bool:
        ...
        try:
            ...
            self.log_test("Error Classes", True, "ClipboardError and ValidationException available")
            ...
            try:
                ...
                self.log_test("Validation Error Handling", False, "Large text should have failed validation")
                return False
            except ValidationException:
                self.log_test("Validation Error Handling", True, "Large text properly rejected")
                return True
            except Exception as e:
                self.log_test("Validation Error Handling", False, f"Unexpected error: {e}")
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Verify MCP Clipboard Server installation")
    parser.add_argument("--package-name", default="mcp-clipboard-server", 
