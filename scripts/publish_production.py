...
def validate_version_consistency():
    ...
    if pyproject_file.exists():
        ...
        if f'version = "{version}"' not in content:
            print(f"Error: Version mismatch in pyproject.toml")
            return False
    ...
    if init_file.exists():
        ...
        if expected_fallback not in content:
            print(f"Warning: Fallback version in __init__.py may be outdated")
...
def check_package_availability():
    ...
    try:
        import requests
        response = requests.get(f"https://pypi.org/pypi/mcp-clipboard-server/{version}/json", timeout=10)
        if response.status_code == 200:
            ...
            print(f"Warning: Could not check PyPI availability (status: {response.status_code})")
...
def verify_pypi_installation():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_result = run_command(test_cmd, check=False, capture_output=True, timeout=30)
...
def main():
    ...
    version = get_package_version()
    print(f"\n=== Ready to Publish ===")
    print(f"Package: mcp-clipboard-server")
    print(f"Version: {version}")
    print(f"Target: https://pypi.org/")
